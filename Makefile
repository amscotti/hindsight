...
lint:
	@if ! $(GOLANGCI_LINT) --version 2>/dev/null | grep -q "$(GOLANGCI_VERSION)"; then \
		echo "want golangci-lint $(GOLANGCI_VERSION) (CI pins v1.64.5 for the v1 config schema); have: $$($(GOLANGCI_LINT) --version 2>/dev/null || echo missing)"; \
		echo "install: GOBIN=\$$$$(go env GOPATH)/bin go install github.com/golangci/golangci-lint/cmd/golangci-lint@$(GOLANGCI_VERSION)"; \
		exit 1; \
	fi
	$(GOLANGCI_LINT) run ./...
